...
# Custom messages
#from find_moving_objects.msg import MovingObject, MovingObjectArray 
...
class DetectedCluster:
    ...
    """
    def __init__(self, pos_x, pos_y, vel_x, vel_y, confidence, in_free_space):
        """
        Constructor
        """
        self.pos_x = pos_x
...
class ObjectTracked:
    ...
    def __init__(self, x, y, vel_x, vel_y, now, confidence, is_person, in_free_space): 
        """
        Constructor
        """        
        self.id_num = ObjectTracked.new_object_id_num
        ObjectTracked.new_object_id_num += 1
...
class KalmanMultiTracker:    
    ...
    def __init__(self):      
        ...
        self.publish_moving_objects_frame = rospy.get_param("publish_moving_objects_frame", self.fixed_frame)
    	...
    	# ROS publishers
        self.moving_objects_tracked_pub = rospy.Publisher('moving_objects_tracked', MovingObjectArray, queue_size=300)
        #self.moving_objects_detected_pub = rospy.Publisher('moving_objects_detected', MovingObjectArray, queue_size=300)
        ...
        # ROS subscribers         
        self.detected_clusters_sub = rospy.Subscriber('moving_objects', MovingObjectArray, self.detected_clusters_callback)      
    ...
    def detected_clusters_callback(self, detected_clusters_msg):    
        """
        Callback for every time find_moving_objects publishes new sets of detected clusters. 
        ...
        """
        if (len(detected_clusters_msg.objects) > 0):
            now = detected_clusters_msg.objects[0].header.stamp # patch: hay que editar el mensaje movingObjects
        else:
            now = 0"""
        now = detected_clusters_msg.header.stamp
        detected_clusters = []
        detected_clusters_set = set()
        for cluster in detected_clusters_msg.objects:
            new_detected_cluster = DetectedCluster(
                cluster.position_in_map_frame.x, 
                cluster.position_in_map_frame.y,
                cluster.velocity_in_map_frame.x,
                cluster.velocity_in_map_frame.y, 
                cluster.confidence, 
                in_free_space=self.how_much_in_free_space(cluster.position_in_map_frame.x, cluster.position_in_map_frame.y)
            )      
        ...
        for detect in detected_clusters:      
            if not detect in matched_tracks.values():
                self.objects_tracked.append(ObjectTracked(detect.pos_x, detect.pos_y, detect.vel_x, detect.vel_y, now, detect.confidence, is_person=False, in_free_space=detect.in_free_space))
    ...
    def publish_tracked_objects(self, now):
        ...
        """        
        marker_id = 0
        objects_tracked_msg = MovingObjectArray()
        #objects_tracked_msg.header.stamp = now
        #objects_tracked_msg.header.frame_id = self.publish_moving_objects_frame
        # Make sure we can get the required transform first:
        if self.use_scan_header_stamp_for_tfs:
            tf_time = now
            try:
                self.listener.waitForTransform(self.publish_moving_objects_frame, self.fixed_frame, tf_time, rospy.Duration(1.0))
                transform_available = True
            except:
                transform_available = False
        else:
            tf_time = rospy.Time(0)
            transform_available = self.listener.canTransform(self.publish_moving_objects_frame, self.fixed_frame, tf_time)

        marker_id = 0
        if not transform_available:
            rospy.loginfo("Objects tracker: tf not avaiable. Not publishing objects")
        else:
            for objects in self.objects_tracked:
                if self.publish_occluded or objects.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                    # Get position in the <self.publish_moving_objects_frame> frame 
                    ps = PointStamped()
                    ps.header.frame_id = self.fixed_frame
                    ps.header.stamp = tf_time
                    ps.point.x = objects.pos_x
                    ps.point.y = objects.pos_y
                    try:
                        ps = self.listener.transformPoint(self.publish_moving_objects_frame, ps)
                    except:
                        rospy.logerr("Not publishing people due to no transform from fixed_frame-->publish_moving_objects_frame")                                                
                        continue
                    
                    # publish to people_tracked topic
                    new_objects = MovingObject() 
                    new_objects.position_in_map_frame.x = ps.point.x 
                    new_objects.position_in_map_frame.y = ps.point.y
                    new_objects.velocity_in_map_frame.x = round(objects.vel_x,2)
                    new_objects.velocity_in_map_frame.y = round(objects.vel_y,2) 
                    new_objects.confidence = objects.confidence
                    ...
                    #new_objects.header = str(objects.id_num) 
                    objects_tracked_msg.objects.append(new_objects)

                    # publish rviz markers       
                    # Cylinder for body 
                    marker = Marker()
                    marker.header.frame_id = self.publish_moving_objects_frame
                    marker.header.stamp = now
                    marker.ns = "People_tracked"
                    marker.color.r = objects.colour[0]
                    marker.color.g = objects.colour[1]
                    marker.color.b = objects.colour[2]          
                    marker.color.a = (rospy.Duration(3) - (rospy.get_rostime() - objects.last_seen)).to_sec()/rospy.Duration(3).to_sec() + 0.1
                    ...
                    marker.text = str(objects.id_num)
                    marker.scale.z = 0.2         
                    ...
                    end_point.x = start_point.x + 0.5*objects.vel_x
                    end_point.y = start_point.y + 0.5*objects.vel_y
                    ...
                    # <self.confidence_percentile>% confidence bounds of object's position as an ellipse:
                    cov = objects.filtered_state_covariances[0][0] + objects.var_obs # cov_xx == cov_yy == cov
                    std = cov**(1./2.)
        ...
        self.moving_objects_tracked_pub.publish(objects_tracked_msg)            


if __name__ == '__main__':
    rospy.init_node('multi_objects_tracker', anonymous=True)
